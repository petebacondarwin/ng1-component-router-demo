    ...
    function outletLink(scope, element, attrs, ctrls, $transclude) {
        var Outlet = (function () {
            ...
            Outlet.prototype.deactivate = function (instruction) {
                if (this.currentComponent && this.currentComponent.$routerOnDeactivate) {
                  var currentComponent = this.currentComponent;
                    return $q.when(currentComponent.$routerOnDeactivate(instruction, this.currentInstruction))
                             .then(function() { delete currentComponent.$router; });
                }
                return $q.when();
            };
            Outlet.prototype.activate = function (instruction) {
                ...
                this.ngOutletController.$$routeParams = instruction.params;
                this.ngOutletController.$$template = '<' + dashCase(componentName) + '></' + dashCase(componentName) + '>';
                ...
                // by debug mode
                this.currentComponent = this.currentElement.children().eq(0).controller(componentName);
                if (this.currentComponent) {
                    this.currentComponent.$router = this.ngOutletController.$$router;
                    if (this.currentComponent.$routerOnActivate) {
                       return this.currentComponent.$routerOnActivate(instruction, previousInstruction);
                    }
                }
                return $q.when();
            };
